...
export function ApplicationReviewForm({ applicationId, requestedRole, brands }: { applicationId: string; requestedRole: "ae" | "advertiser"; brands: { id: string; name: string }[] }) {
  ...
  return <form action={action} className="application-review-form">
    ...
    <label className="field">
      <span>{requestedRole === "ae" ? "담당 브랜드" : "소속 브랜드"}</span>
      <select name="brandId" required={requestedRole === "ae"}>
        <option value="">{requestedRole === "ae" ? "배정할 브랜드 선택" : "신규 가상 브랜드 생성"}</option>
    ...
    <div className="form-actions">
      <button className="primary-button" name="decision" value="approved" disabled={pending}>{pending ? "처리 중…" : "승인"}</button>
      <button className="danger-button" name="decision" value="rejected" disabled={pending}>거절</button>
    </div>
