...
export function TestPublishForm({ contentId, versions, publishedVersionId }: { contentId: string; versions: Array<{ id: string; version_no: number; change_summary: string }>; publishedVersionId: string | null }) {
  ...
  return <section className="test-publish-card" aria-labelledby="test-publish-title"><div><h2 id="test-publish-title">테스트 발행</h2><p>명시적으로 확정한 불변 버전만 로컬 테스트 자사몰에 노출합니다. 실제 승인이나 운영 발행이 아닙니다.</p></div>
    ...
    {state.message && <p role="status" className={state.ok ? "success-message" : "form-error"}>{state.message}</p>}
  </section>;
}
