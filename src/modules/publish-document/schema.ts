...
const relatedProductSchema = z.object({ id: z.string().min(1).max(80), name: z.string().min(1).max(120), description: z.string().max(240), category: z.string().max(80), url: z.url() }).strict();
...
export const publishDocumentSchema = z.object({
  ...
  faq: z.array(faqSchema).max(20), sources: z.array(sourceSchema).max(30), cta: ctaSchema.nullable(), relatedProducts: z.array(relatedProductSchema).max(20),
