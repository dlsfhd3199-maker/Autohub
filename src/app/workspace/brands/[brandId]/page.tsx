...
import { PublishingConnectionPanel } from "@/components/publishing-connection-panel";
import { getPublishingConnection } from "@/modules/publishing/queries";
...
export default async function BrandDetailPage({ params }: { params: Promise<{ brandId: string }> }) {
  ...
  const connection = context.role === "agency_admin" ? await getPublishingConnection(brandId) : null;
  return <main className="workspace-content"><Link className="back-link" href="/workspace/brands">← 브랜드 목록</Link>
    ...
    <section className="panel detail-panel"><div className="detail-summary"><div><small>브랜드 키</small><strong>{brand.brand_key}</strong></div><div><small>연결 도메인</small><strong>{brand.domain}</strong></div><div><small>기본 발행 경로</small><strong>{brand.publishing_path}</strong></div><div><small>최근 수정</small><strong>{formatKoreanDate(brand.updated_at)}</strong></div></div>
      ...
      <div className="brand-detail-actions"><Link className="secondary-button" href={`/workspace/brands/${brand.id}/site-crawl`}>사이트 수집</Link><Link className="secondary-button" href={`/workspace/brands/${brand.id}/knowledge`}>브랜드 지식</Link><Link className="secondary-button" href={`/workspace/brands/${brand.id}/products`}>상품 정보</Link></div>
    </section>{context.role === "agency_admin" && <PublishingConnectionPanel brandId={brandId} status={(connection?.status as "active" | "disabled" | undefined) ?? null} />}</main>;
}
