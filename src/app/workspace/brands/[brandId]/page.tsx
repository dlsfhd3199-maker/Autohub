...
export default async function BrandDetailPage({ params }: { params: Promise<{ brandId: string }> }) {
  ...
  return <main className="workspace-content"><Link className="back-link" href="/workspace/brands">← 브랜드 목록</Link><section className="page-heading detail-heading"><div><p className="eyebrow">BRAND DETAIL</p><h1>{brand.name}</h1><p>{brand.domain}{brand.publishing_path}</p></div><span className={`status-badge ${brand.archived_at ? "status-archived" : "status-active"}`}><span />{brand.archived_at ? "보관" : "운영"}</span></section><section className="panel detail-panel"><div className="detail-summary"><div><small>브랜드 키</small><strong>{brand.brand_key}</strong></div><div><small>연결 도메인</small><strong>{brand.domain}</strong></div><div><small>기본 발행 경로</small><strong>{brand.publishing_path}</strong></div><div><small>최근 수정</small><strong>{formatKoreanDate(brand.updated_at)}</strong></div></div>{context.role === "agency_admin" ? <BrandEditForm brand={brand} organizationId={context.organizationId} /> : <div className="readonly-note"><Icon name="warning" /><span><strong>읽기 전용 화면입니다.</strong> 브랜드 정보 변경은 대행사 관리자만 가능합니다.</span></div>}</section></main>;
}
